...
class LowerCaseInputStream extends FilterInputStream {
    ...
    public int read() throws IOException {
        int c = super.read();
        return (c == -1) ? c : Character.toLowerCase(c); // 대문자화!
    }
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        ...
        if (result != -1) {
            for (int i = off; i < off + result; i++) {
                b[i] = (byte) Character.toLowerCase((char) b[i]); // 대문자화!
            }
        }
        return result;
    }
...
public class DecoratorExample {
    ...
    public static void main(String[] args) {
        try (FileInputStream fileInputStream = new FileInputStream("src/decorator/ex2_inputstream/example.txt");
             ...
             InputStream inputStream = new LowerCaseInputStream(new UpperCaseInputStream(bufferedInputStream))) {
